        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="mb-8 fade-in">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-4xl font-bold text-gray-900 mb-2">Laporan Peringkat Produk</h1>
                        <p class="text-gray-600">Produk dengan rating tertinggi</p>
                    </div>
                    <div class="text-right no-print">
                        <p class="text-sm text-gray-500">{{ now()->format('d F Y') }}</p>
                        <p class="text-2xl font-bold text-purple-600">{{ $products->count() }}</p>
                        <p class="text-xs text-gray-500">Produk Teratas</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl shadow-md p-6 mb-6 fade-in no-print">
                <form method="GET" action="{{ route('admin.reports.product-ranking') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="limit" class="w-full rounded-lg border-gray-300 focus:border-purple-500 focus:ring-purple-500">
                    ...
                    <div class="flex items-end space-x-2 md:col-span-2">
                        ...
                        <a href="{{ route('admin.reports.product-ranking.export', request()->all()) }}" class="bg-green-600 text-white px-4 py-2.5 rounded-lg hover:bg-green-700 transition inline-flex items-center">
            ...
            <div class="bg-white rounded-2xl shadow-md overflow-hidden fade-in">
                <div class="p-6 border-b border-gray-200">
                    <h2 class="text-lg font-bold text-gray-900">Peringkat Produk</h2>
                    <p class="text-sm text-gray-500">Urutan berdasarkan rating tertinggi</p>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Jumlah Review</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($index < 3)
                                        <div class="flex items-center justify-center w-10 h-10 rounded-full {{ $index == 0 ? 'bg-yellow-100' : ($index == 1 ? 'bg-gray-100' : 'bg-orange-100') }}">
                                            <span class="text-xl">{{ $index == 0 ? '🥇' : ($index == 1 ? '🥈' : '🥉') }}</span>
                                        </div>
                                    @else
                                        <div class="text-center text-gray-600 font-semibold">{{ $index + 1 }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <i class="uil uil-star text-yellow-400 mr-1"></i>
                                        <span class="text-lg font-bold text-gray-900">{{ number_format($product->avg_rating, 2) }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="text-sm font-medium text-gray-600">{{ $product->review_count }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
